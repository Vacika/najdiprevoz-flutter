                  ...
                  children: <Widget>[
                    ListTile(
                      ...
                      title: Container(
                          ...
                          child:Text(req.requesterName)),
                      leading: circleImage(image),
                      subtitle: Column(
                          ...
                          children: [
                            Row(
                                ...
                                children: [Text(
                                  ...
                                  IconButton(
                                      onPressed: () {}, icon: Icon(getIcon(req),
                                      color: getColor(req)))
                                ]),
                            Divider(),
    ...
    return req.allowedActions
        ...
        TextButton(
            ...
            onPressed: () =>
            {
              setState(() {
                var updatedReq = req;
                updatedReq.allowedActions = [];
                updatedReq.requestStatus = e.toUpperCase() + 'D';
                reservations[reservations.indexOf(req)] = updatedReq;
              })
            }))
        .toList();
  }

  getColor(ReservationRequestFullResponse req) {
    return req.requestStatus == 'APPROVED' ? Colors.green : (req
        .requestStatus == 'DECLINED' ? Colors.red : Colors.pinkAccent);
  }

  IconData getIcon(ReservationRequestFullResponse req) {
    return req.requestStatus == 'APPROVED' ? Icons.check : (req.requestStatus ==
        'PENDING' ? Icons.notifications_active_rounded : Icons.dangerous);
    }
}
